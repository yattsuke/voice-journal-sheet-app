...
アプリでは、録音前に次のテーマを選べます。

- `仕事` -> `仕事` シート
- `生活` -> `生活` シート
- `その他` -> `その他` シート

必要なら `lib/journal-themes.ts` を編集して、テーマ名や保存先シート名を変えられます。

## Google スプレッドシート連携

1. 新しい Google スプレッドシートを作成
2. 必要なら `仕事` `生活` `その他` のシートを先に作成する  
   作らなくても、初回保存時に自動作成されます
3. 各シートの 1 行目に次のヘッダーを入れる
...
const HEADERS = ["recordedAt", "polishedTitle", "polishedBody", "transcript"];
...
function doPost(e) {
  ...
  const sheetName = payload.sheetName || "その他";
...
6. Web アプリとしてデプロイ
7. 発行された URL を `GOOGLE_APPS_SCRIPT_URL` に設定
8. `SECRET` と `GOOGLE_APPS_SCRIPT_SECRET` を同じ値にする
